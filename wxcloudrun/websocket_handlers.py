...
from wxcloudrun import socketio, db
...
def handle_join_room(data):
    ...
    try:
        ...
        join_room(room)
        emit('room_joined', {'code': 0, 'data': {'room': room, 'message': '成功加入房间'}})
        
    except Exception as e:
        emit('error', {'code': -1, 'errorMsg': f'加入房间失败: {str(e)}'})
...
def handle_leave_room(data):
    ...
    try:
        ...
        room = f"{user_id}_{session_id}"
        leave_room(room)
        emit('room_left', {'code': 0, 'data': {'room': room, 'message': '成功离开房间'}})
        
    except Exception as e:
        emit('error', {'code': -1, 'errorMsg': f'离开房间失败: {str(e)}'})
